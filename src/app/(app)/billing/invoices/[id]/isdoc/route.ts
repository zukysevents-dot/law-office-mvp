...
// Content-Disposition must stay ASCII; derive a safe filename from the number.
function isdocFilename(invoiceNumber: string | null): string {
...
export async function GET(
  ...
  // Same visibility gate as the invoice detail page — org isolation + role
  // visibility are enforced here, so export never exposes more than viewing.
  ...
  let xml: string;
  try {
    xml = buildIsdocXml(mapInvoiceToIsdocInput(invoice));
  } catch (error) {
  ...
  return new Response(xml, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${isdocFilename(invoice.number)}"`,
    },
  });
}
